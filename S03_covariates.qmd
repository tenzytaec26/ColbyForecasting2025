...
```{r filter_collinear}
keep = filter_collinear(apr, method = "vif_step")
keep
```
...
```{r extract_aug}
variables = extract_brickman(apr, model_input, form = "wide")
variables
```
...
for (month in month.abb){
  greedy = select_covariates(approach = "greedy", mon = "Jun")
  
  conservative = select_covariates(approach = "conservative" , mon = "Jun")
