...
```{r load_covar}
cfg = read_configuration(version = "g_Jun")
...
plot(nowcast['.pred_presence'], main = "Nowcast June", 
...
```{r plot_class_labels}
plot(nowcast['.pred'], main = "Nowcast June Labels", 
...
write_prediction(nowcast, file = file.path(path,"g_Jun_RCP85_2020.tif"))
write_prediction(forecast_2055, file = file.path(path, "g_Jun_RCP85_2055.tif"))
write_prediction(forecast_2075, file = file.path(path, "g_Jun_RCP85_2075.tif"))

# or write them together in a "multi-layer" file?
write_prediction(rcp85, file = file.path(path, "g_Jun_RCP85_all.tif"))
```
...
```{r forecast}
forecast_2075_45 = predict_stars(wflow, covars_rcp45_2075)
forecast_2075_45
```

```{r plot_forecast}
coast = read_coastline()
plot(forecast_2075_45['.pred_presence'], main = "RCP45 2075 June", 
     axes = TRUE, breaks = seq(0, 1, by = 0.1), reset = FALSE)
plot(coast, col = "orange", lwd = 2, add = TRUE)
```
```{r plot_class_labels}
plot(forecast_2075_45['.pred'], main = "RCP45 2075 June Labels", 
     axes = TRUE, reset = FALSE)
plot(coast, col = "black", lwd = 2, add = TRUE)
```
```{r load_2055_RCP45}
...
```{r forecast}
forecast_2055_45 = predict_stars(wflow, covars_rcp45_2055)
forecast_2055_45
```
...
```{r plot_forecast}
coast = read_coastline()
plot(forecast_2055_45['.pred_presence'], main = "RCP45 2055 June", 
     axes = TRUE, breaks = seq(0, 1, by = 0.1), reset = FALSE)
plot(coast, col = "orange", lwd = 2, add = TRUE)
```
...
```{r plot_class_labels}
plot(forecast_2055_45['.pred'], main = "RCP45 2055 June Labels", 
     axes = TRUE, reset = FALSE)
plot(coast, col = "black", lwd = 2, add = TRUE)
```
